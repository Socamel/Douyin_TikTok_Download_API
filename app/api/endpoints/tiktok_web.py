...
from crawlers.tiktok.web.web_crawler import TikTokWebCrawler  # 导入TikTokWebCrawler
...
async def fetch_one_video(request: Request,
    ...
    """
    try:
        data = await tiktok_web_crawler.fetch_one_video(itemId)
...
@router.get("/fetch_user_profile",
            response_model=ResponseModel,
            summary="获取用户的个人信息Get user profile")
async def fetch_user_profile(request: Request,
    ...
    """
    try:
        data = await tiktok_web_crawler.fetch_user_profile(secUid, uniqueId)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
@router.get("/fetch_user_post",
            response_model=ResponseModel,
            summary="获取用户的作品列表Get user posts")
async def fetch_user_post(request: Request,
                          ...
                          cursor: int = Query(default=0, description="翻页游标标/Page cursor"),
    ...
    """
    try:
        data = await tiktok_web_crawler.fetch_user_post(secUid, cursor, count, coverFormat)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
@router.get("/fetch_user_like",
            response_model=ResponseModel,
            summary="获取用户的点赞列表Get user likes")
async def fetch_user_like(request: Request,
    ...
    """
    try:
        data = await tiktok_web_crawler.fetch_user_like(secUid, cursor, count, coverFormat)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
@router.get("/fetch_user_collect",
            response_model=ResponseModel,
            summary="获取用户的收藏列表Get user favorites")
async def fetch_user_collect(request: Request,
    ...
    ### 用途
    - 获取用户的收藏列表
    - 注意: 该接口目前只能获取自己的收藏列表表，需要提供自己账号的cookie?
    ### 参数:
    - cookie: 用户cookie
    - secUid: 用户secUid
    - cursor: 翻页游标标
    ...
    """
    try:
        data = await tiktok_web_crawler.fetch_user_collect(cookie, secUid, cursor, count, coverFormat)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
@router.get("/fetch_user_play_list",
            response_model=ResponseModel,
            summary="获取用户的播放列表Get user play list")
async def fetch_user_play_list(request: Request,
    ...
    """
    try:
        data = await tiktok_web_crawler.fetch_user_play_list(secUid, cursor, count)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
@router.get("/fetch_user_mix",
            response_model=ResponseModel,
            summary="获取用户的合辑列表Get user mix list")
async def fetch_user_mix(request: Request,
    ...
    """
    try:
        data = await tiktok_web_crawler.fetch_user_mix(mixId, cursor, count)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
@router.get("/fetch_post_comment",
            response_model=ResponseModel,
            summary="获取作品的评论列表Get video comments")
async def fetch_post_comment(request: Request,
                             ...
                             current_region: str = Query(default="", description="当前地区区/Current region")):
    """
    # [中文]
    ### 用途
    - 获取作品的评论列表
    ...
    """
    try:
        data = await tiktok_web_crawler.fetch_post_comment(aweme_id, cursor, count, current_region)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
@router.get("/fetch_post_comment_reply",
            response_model=ResponseModel,
            summary="获取作品的评论回复列表Get video comment replies")
async def fetch_post_comment_reply(request: Request,
    ...
    """
    try:
        data = await tiktok_web_crawler.fetch_post_comment_reply(item_id, comment_id, cursor, count, current_region)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
@router.get("/fetch_user_fans",
            response_model=ResponseModel,
            summary="获取用户的粉丝列表Get user followers")
async def fetch_user_fans(request: Request,
                          secUid: str = Query(example="MS4wLjABAAAAv7iSuuXDJGDvJkmH_vz1qkDZYo1apxgzaxdBSeIuPiM",
                                              description="用户secUid/User secUid"),
                          count: int = Query(default=30, description="每页数量/Number per page"),
                          maxCursor: int = Query(default=0, description="最大游标Max cursor"),
                          minCursor: int = Query(default=0, description="最小游标Min cursor")):
    """
    # [中文]
    ### 用途
    - 获取用户的粉丝列表
    ...
    """
    try:
        data = await tiktok_web_crawler.fetch_user_fans(secUid, count, maxCursor, minCursor)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
@router.get("/fetch_user_follow",
            response_model=ResponseModel,
            summary="获取用户的关注列表Get user followings")
async def fetch_user_follow(request: Request,
    ...
    """
    try:
        data = await tiktok_web_crawler.fetch_user_follow(secUid, count, maxCursor, minCursor)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def generate_real_msToken(request: Request):
    ...
    """
    try:
        data = await tiktok_web_crawler.fetch_real_msToken()
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def generate_ttwid(request: Request,
    ...
    """
    try:
        data = await tiktok_web_crawler.fetch_ttwid(cookie)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def generate_xbogus(request: Request,
                          url: str = Query(
                              ...
                              description="未签名称的API URL/Unsigned API URL"),
    ...
    """
    try:
        data = await tiktok_web_crawler.gen_xbogus(url, user_agent)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
@router.get("/get_sec_user_id",
            response_model=ResponseModel,
            summary="提取列表表用户id/Extract list user id")
async def get_sec_user_id(request: Request,
    ...
    """
    try:
        data = await tiktok_web_crawler.get_sec_user_id(url)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def get_all_sec_user_id(request: Request,
    ...
    """
    try:
        data = await tiktok_web_crawler.get_all_sec_user_id(url)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def get_aweme_id(request: Request,
    ...
    """
    try:
        data = await tiktok_web_crawler.get_aweme_id(url)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
@router.post("/get_all_aweme_id",
             response_model=ResponseModel,
             summary="提取列表表作品id/Extract list video id")
async def get_all_aweme_id(request: Request,
    ...
    """
    try:
        data = await tiktok_web_crawler.get_all_aweme_id(url)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
async def get_unique_id(request: Request,
    ...
    """
    try:
        data = await tiktok_web_crawler.get_unique_id(url)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
...
@router.post("/get_all_unique_id",
             response_model=ResponseModel,
             summary="获取列表表unique_id/Get list unique_id")
async def get_all_unique_id(request: Request,
    ...
    """
    try:
        data = await tiktok_web_crawler.get_all_unique_id(url)
        return ResponseModel(code=200,
                             router=request.url.path,
                             data=data)
    except Exception as e:
