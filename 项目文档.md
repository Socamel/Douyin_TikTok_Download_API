...
```bash
# 测试Cookie状�?
curl -X GET http://localhost:80/api/cookie/status

# 测试Cookie更新
curl -X POST http://localhost:80/api/cookie/update_cookie \
...
```bash
# 测试新增Blogger
curl -X POST http://localhost:80/api/aitable/blogger_cookie \
  -H "Content-Type: application/json" \
  -d '{
    "recordId": "test_001",
    "fields": {
...
- `test_aitable_api.py` - Python测试脚本
...
# 检查数据库状�?
curl -X POST http://localhost:80/api/aitable/sync_status
```
